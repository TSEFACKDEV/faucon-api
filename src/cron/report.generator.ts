...
export const computeDayStats = async (
  ...
  const nbAlarmes = await prisma.alarme.count({
    where: {
      vehiculeId,
      horodatage: { gte: start, lte: end },
    },
  ...
  for (let i = 1; i < positions.length; i++) {
    ...
    // Vitesse max
    if (curr.vitesse > vitesseMax) vitesseMax = curr.vitesse;
    ...
    if (prev.vitesse <= SEUIL_ARRET_KMH && curr.vitesse <= SEUIL_ARRET_KMH) {
      const diffMs =
        new Date(curr.horodatage).getTime() -
 ...
 */
export const generateAllReports = async (date: Date): Promise<void> => {
  ...
  await Promise.allSettled(
    vehicules.map(v => generateVehicleReport(v.id, date))
  );
